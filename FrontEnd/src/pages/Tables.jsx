...
const Tables = () => {
  ...
  // Simple inline filtering like Orders
  const filteredTables =
    status === "all"
      ? tables
      : tables.filter(table =>
          status === "booked" ? table.status === "Booked"
      ...
      <div className='flex items-center justify-between px-10 py-4'>
        ...
        <div className='flex items-center gap-2 bg-[#1a1a1a] p-1 rounded-xl border border-white/5'>
          ...
          <button 
            ...
            className={`px-5 py-2 text-sm font-semibold transition-all rounded-lg ${status === "available" ? "bg-[#383838] text-white" : "text-[#ababab]"}`}
          >
            Available
          </button>
          <button 
      ...
      <div className='flex-1 overflow-y-auto scrollbar-hide px-10 pb-32 h-[calc(100vh-9rem)]'>
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 justify-items-center'>
          {filteredTables.length > 0 ? (
            ...
            <p className='text-gray-400 col-span-full'>No tables found</p>
          )}
        </div>
      </div>
